...
void deploy_z_probe() {
  feedrate = homing_feedrate[X_AXIS];//coordinates deploy z probe 
  destination[X_AXIS] = 101;//40;//20;//20//40   // G1 X114 Y-50 Z65
  destination[Y_AXIS] = -45;//106;//128;//118 potlood
  destination[Z_AXIS] = 39;//45;//50
  prepare_move_raw();

  feedrate = homing_feedrate[X_AXIS]/10;//moving for deploy .speed
  //destination[X_AXIS] = 30;//0;//moving to coordinate x30 to deploy back tower
  destination[Y_AXIS] = -30;//infront delta right,
  prepare_move_raw();
  st_synchronize();
}

void retract_z_probe() { // back move from last measurepiont was digging in glass raised a bit
  feedrate = homing_feedrate[X_AXIS];
  destination[Z_AXIS] = current_position[Z_AXIS] + 35; // was + 20;
  prepare_move_raw();
// cordinates of screw to retract z-probe
  destination[X_AXIS] = 109; //-44;     // G1 X107 Y22.5 Z43
  destination[Y_AXIS] = 14; //112; // was 63;
  destination[Z_AXIS] = 50;//30; //was 30;
  prepare_move_raw();

  // Move the nozzle below the print surface to push the probe up.
  feedrate = homing_feedrate[Z_AXIS]/10;
  destination[Z_AXIS] = current_position[Z_AXIS] - 16; //- 22; //was - 20;
  prepare_move_raw();
